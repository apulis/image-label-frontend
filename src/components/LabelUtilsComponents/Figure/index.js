...
import { Polyline, Polygon, CircleMarker, Marker, Tooltip, Popup } from 'react-leaflet';
import './index.less';
...
class Figure extends Component {
  ...
  render() {
    const { figure, options, skipNextClick, isOCR } = this.props;
    const { id, points, isM, type, popupText } = figure;
    const {
      editing,
      finished,
      sketch,
    ...
    const { draggedPoint, popupPoint } = this.state;
    const renderPoints = this.getRenderPoints(points);
    const dashArray = editing || isM ? '10' : '1';
